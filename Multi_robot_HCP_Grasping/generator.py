...
def robot_param_random_sample(robot, pre_gen_params, vars_to_change):
    ...
    if '1j' in robot.name:
        ...
        for f_idx , j_idx in zip(link_idx,joint_idx):
            if vars_to_change['layer_rad']:
                radius_id =np.random.randint(0, len(pre_gen_params['radius']),1)[0]
                radius = pre_gen_params['radius'][radius_id]
            geo_change(robot, f_idx, radius, length)
            robot.joints[j_idx].origin[2][3] = length
    elif '2f2j' in robot.name:
        #1st layer:
        for f_idx , j_idx in zip(link_idx[::2],joint_idx[::2]):
            if vars_to_change['layer_rad']:
                radius_id =np.random.randint(0, len(pre_gen_params['radius']),1)[0]
                radius = pre_gen_params['radius'][radius_id]
            geo_change(robot, f_idx, radius, length)
        ...
        for f_idx, j_idx in zip(link_idx[1::2],joint_idx[1::2]):
            if vars_to_change['layer_rad']:
                radius_id =np.random.randint(0, len(pre_gen_params['radius']),1)[0]
                radius = pre_gen_params['radius'][radius_id]
            geo_change(robot, f_idx, radius, length)
            robot.joints[j_idx].origin[2][3] = length
    elif '3f2j' in robot.name:
        #1st layer:
...
def geo_change(robot, idx, r, l):
    midpoint = l/2
    robot.links[idx].collisions[0].geometry.cylinder = None
    robot.links[idx].collisions[0].geometry.box = urdfpy.urdf.Box([r*2, r*2, l])
    robot.links[idx].collisions[0].origin[2][3] = midpoint
    robot.links[idx].visuals[0].geometry.cylinder = None
    robot.links[idx].visuals[0].geometry.box = urdfpy.urdf.Box([r*2, r*2, l])
...
def gen_robot_configs(ref_file, robot_num,
    ...
    for idx in tqdm(range(robot_num)):
        ...
        sub_save_dir = os.path.join(root_save_dir, f'robot_{robot_model}_{idx}')
...
if __name__ == "__main__":
    ...
    ref_urdf = f'../assets/ur5_w_gripper/{args.model}.urdf'
    save_dir = f'../assets/multi_robot_gen_gripper/'
